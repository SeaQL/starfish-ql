...
use sea_query::{Alias, Cond, Expr, OnConflict, Query, QueryStatementBuilder, SimpleExpr};
...
impl Mutate {
    ...
    pub async fn insert_edge_batch(
        ...
        let columns = [Alias::new("from_node"), Alias::new("to_node")];
        let mut stmt = Query::insert();
        stmt.into_table(Alias::new(&format_edge_table_name(edge_json_batch.of)))
            .columns(columns.clone())
            .on_conflict(
                OnConflict::columns(columns.clone())
                    .do_nothing()
                    .to_owned(),
            );
